<div class="card">
    <div class="card-header">
        ...
        <button class="btn btn-secondary btn-sm pull-right" data-bs-toggle="modal" data-bs-target="#createrole">Create
            New</button>
    </div>
            ...
            @foreach ($roles as $role)
                <tr>
                    ...
                    <td>
                        ...
                        <button type="button" class="btn btn-sm btn-primary pull-left" data-bs-toggle="modal"
                ...
                <form method="POST" action="{{ route('admin.role.create') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="description"
                            placeholder="Allow anyone to do anything on the site">
                    </div>
                    <div class="row">
                        <div class="col-6">
                            <div class="mb-3">
                                ...
                                <div class="form-text">This software is using <a
                                        href="https://feathericons.com/">feather icons</a>. Learn more on their site.
                                </div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="mb-3">
                                ...
                                <input type="color" class="form-control colorpicker br-3" name="color"
                                    style="border: none;">
                            </div>
                        </div>
                    </div>
                                ...
                                @if ($group == null)
                                    ...
                                    <div class="col-4 mb-1">
                                        <div class="form-check form-switch big-checkbox">
                                            <input name="{{ $permission->key }}" class="form-check-input check"
                                                type="checkbox" id="flexSwitchCheckDefault">
                                            <label class="form-check-label" for="flexSwitchCheckDefault"
                                                style="text-transform: capitalize;">{{ $permission->key }}</label>
                                        </div>
                                    </div>
...
@foreach ($roles as $role)
    <div class="modal fade" id="editrole-{{ $role->id }}" tabindex="-1" aria-labelledby="editroleLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                        ...
                        <div class="row">
                            <div class="col-6">
                                <div class="mb-3">
                                    <label for="exampleFormControlInput1" class="form-label">Role Icon:</label>
                                    <div class="input-group">
                                        <span class="input-group-text"><i id="msgbox"
                                                data-feather="{{ $role->icon }}"></i></span>
                                        <select class="form-control" id="icons" name="icon">
                                            <option selected>Select an Icon</option>
                                            @for ($x = 0; $x <= count($icons) - 1; $x++)
                                                <option @if ($role->icon == $icons[$x]) selected @endif value="{{ $icons[$x] }}">
                                                    {{ $icons[$x] }}</option>
                                            @endfor
                                        </select>
                                    </div>
                                    <div class="form-text">This software is using <a
                                            href="https://feathericons.com/">feather icons</a>. Learn more on their
                                        site.</div>
                                </div>
                            ...
                            <div class="col-6">
                                <div class="mb-3">
                                    <label for="exampleFormControlInput1" class="form-label">Role Color:</label>
                                    <input type="color" class="form-control colorpicker br-3" name="color"
                                        value="{{ $role->color }}" style="border: none;">
                                </div>
                            </div>
                        </div>

                        <label for="exampleFormControlInput1" class="form-label">Role Permissions:</label>
                        <div class="row">
                            @foreach ($permissions as $permission)
                            @php
                                $use = 0;
                                        ...
                                        @if ($perm->role_id == $role->id)
                                            <div class="col-4 mb-1">
                                                <div class="form-check form-switch big-checkbox">
                                                    ...
                                                    <label class="form-check-label" for="flexSwitchCheckDefault"
                                                        style="text-transform: capitalize;">{{ $permission->key }}<small
                                                            class="ml-2">({{ $permission->group }})</small></label>
                                                </div>
                                            </div>
                                            @php
                                                $use = 1;
