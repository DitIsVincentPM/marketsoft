    ...
    <div class="card shadow">
      ...
      <div class="card-body pb-0">
        ...
        <div class="row mb-2">
          ...
          <div class="col-2">
            <p class="market-text-break announcement-date">{{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $category->created_at)->format('m/d/Y') }}</p>
          </div>
      ...
      <div class="modal-body">
        <form method="POST" action="{{ route('admin.knowledgebase.update', $knowledgebase->id) }}">
          ...
          <div class="mb-3">
            ...
            <select class="form-select" name="category">
              @foreach($categorys as $category)
              <option @if($category->id == $knowledgebase->category_id) selected @endif value="{{ $category->id }}">{{ $category->name }}</option>
              @endforeach
            </select>
          </div>
      </div>
