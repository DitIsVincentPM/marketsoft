        ...
        <table class="table caption-top admin-table-footer">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $product->price }}</td>
                    <td><button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#viewmore-{{ $product->id }}">View More</button></td>
                </tr>
                @endforeach
            </tbody>
        </table>
...
@foreach($products as $product)
{{-- View More Modal --}}
<div class="modal fade" id="viewmore-{{ $product->id }}" tabindex="-1" aria-labelledby="viewmoreLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="viewmoreLabel">Seller Request #{{ $product->id }}</h4>
            ...
            <div class="modal-body">
                ...
                <div class="input-group mb-3">
                    <div class="input-group-append">
                        <span class="input-group-text" id="basic-addon2">Product Category:</span>
                    </div>
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control" disabled placeholder="{{ $product->price }}">
                </div>
                <hr>
                <form method="post" action="{{ route('admin.sellerrequests.store') }}">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input hidden name="id" value="{{ $product->id }}"></input>
                        <select name="status" class="form-select">
                            <option @if($product->status == 0) selected @endif value="0">Pending</option>
                            <option @if($product->status == 1) selected @endif value="1">Accepted</option>
                            <option @if($product->status == 2) selected @endif value="2">Denied</option>
                        </select>
                    </div>
            </div>
